...
export const AppLayout = ({ children, activeView, onViewChange, onSync, onAddManual, onClearData, onLogout, user, isSyncing }: LayoutProps) => {
    ...
    return (
        <div className="flex h-screen bg-[#0a0a0c] text-slate-100 overflow-hidden">
            {/* Sidebar Desktop */}
            <aside className="hidden md:flex flex-col w-64 border-r border-slate-800 bg-[#0e0e11] p-6">
            ...
            <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
                {/* Mobile Header */}
                <header className="md:hidden flex items-center justify-between p-4 border-b border-slate-800 bg-[#0e0e11]">
            ...
            {isMobileMenuOpen && (
                <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm md:hidden">
                    <aside className="w-64 h-full bg-[#0e0e11] p-6 shadow-2xl animate-in slide-in-from-left duration-300 flex flex-col">
                        ...
                        <div className="bg-white/5 border border-white/5 rounded-2xl p-4 mb-8 flex items-center space-x-3 flex-shrink-0">
                            <div className="w-10 h-10 bg-gradient-to-br from-indigo-500 to-blue-600 rounded-xl flex items-center justify-center font-bold text-white">
                                {user?.name?.[0] || 'U'}
                            </div>
                            <div className="flex-1 min-w-0">
                                <p className="text-sm font-bold truncate">{user?.name || 'Usuario'}</p>
                                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">{user?.email || 'Local'}</p>
                            </div>
                        </div>
                        ...
                        <nav className="space-y-2 flex-1 overflow-y-auto custom-scrollbar pr-2 -mr-2">
                            <button
                                ...
                                className="w-full flex items-center space-x-3 px-4 py-3 rounded-lg bg-indigo-600/10 text-indigo-400 border border-indigo-500/20 mb-4"
                            >
                                <PlusCircle size={20} />
                                <span className="font-semibold">Nuevo Registro</span>
                            </button>
                        ...
                        <div className="mt-auto pt-6 border-t border-slate-800 space-y-4 flex-shrink-0">
                            <button
                                disabled={isSyncing}
                                onClick={onSync}
                                className="w-full flex items-center justify-center space-x-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 disabled:bg-indigo-600/50 rounded-lg font-semibold transition-all"
                            >
                                <RefreshCw className={cn("w-4 h-4", isSyncing && "animate-spin")} />
                                <span>Sincronizar</span>
                            </button>
                            <button
                                ...
                                className="w-full flex items-center justify-center space-x-2 px-4 py-2 bg-rose-500/10 text-rose-400 border border-rose-500/20 rounded-lg font-semibold transition-all"
                            >
                                <Trash2 size={16} />
                                <span>Limpiar Datos</span>
                            </button>
                            <button
                                ...
                                className="w-full flex items-center space-x-3 px-4 py-2 text-slate-500 border border-transparent"
                            >
                                <LogOut size={20} />
                                <span className="font-bold">Cerrar Sesión</span>
                            </button>
